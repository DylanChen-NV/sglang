...
class Fp8Config(QuantizationConfig):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            ...
            if (
                self.is_fp4_experts
                and get_moe_runner_backend().is_flashinfer_humming()
            ):
                from sglang.srt.layers.quantization.mxfp4_flashinfer_humming_moe import (
                    Mxfp4FlashinferHummingMoEMethod,
                )

                return Mxfp4FlashinferHummingMoEMethod(fp8_method, prefix=prefix)
            ...
            if self.is_fp4_experts and get_moe_runner_backend().is_lowlatency_mxfp4():
                from sglang.srt.layers.quantization.mxfp4_lowlatency_moe import (
                    Mxfp4LowLatencyMoEMethod,
                )

                return Mxfp4LowLatencyMoEMethod(fp8_method, prefix=prefix)
